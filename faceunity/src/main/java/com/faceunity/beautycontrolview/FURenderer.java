...
public class FURenderer implements OnFaceUnityControlListener {
    ...
    private int mGaussFS = 15;
    protected int mGaussW = 640;
    protected int mGaussH = 320;
     ...
     * 每帧处理画面时被调用
     */
    private void prepareDrawFrame(byte[] img, int w, int h) {
        //计算FPS等数据
        benchmarkFPS();
        ...
        if(isTracking <= 0) {
//           mGaussBlur.processFrame(img, mGaussW, mGaussH);
        }
